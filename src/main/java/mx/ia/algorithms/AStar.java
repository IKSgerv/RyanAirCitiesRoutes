			...
			for (Edge edge : graphG.getE()) {
				if (bfsElement.equalsStr(edge.getFrom()) && neighborCode.equals(edge.getTo())) {
					if (!contains(bfsClosed, neighborCode)
						...
						elementToAdd = new AStarElement(vElements.get(neighborCode), bfsElement.getLevel() + 1,
								bfsElement.getNeighbors().get(neighborCode) + bfsElement.getWeight(), vTo, bfsElement);
...
//	}
	...
	public double heuristicValuer(Vertex vTo) {
		...
		System.out.println(":::" + vertex.getCode() + ":" + res + "|" + angle + "|" + distance + "|"
				+ ((vertex.getDanger() + 1) * 100));
		return res;
	}
