...
//		System.out.save();
	...
	private Vector<PrimeroElMejorElement> heuristicNeighbors(PrimeroElMejorElement bfsElement, Vertex vTo,
			double angle) {
			...
			for (Edge edge : graphG.getE()) {
				if (bfsElement.equalsStr(edge.getFrom()) && neighborCode.equals(edge.getTo())) {
					if (!contains(bfsClosed, neighborCode)
						...
						elementToAdd = new PrimeroElMejorElement(vElements.get(neighborCode), bfsElement.getLevel() + 1,
								bfsElement.getNeighbors().get(neighborCode) + bfsElement.getWeight(), vTo, angle);
