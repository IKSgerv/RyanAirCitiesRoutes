					...
					if (rules != null) {
						...
						for (String neighborCode : element.getNeighbors().keySet()) {
							if (!contains(ildfsClosed, neighborCode)) {
								elementToAdd = new IterativeLimitedDepthFirstSearchElement(vElements.get(neighborCode),
										element.getLevel() + 1,
...
//		System.out.save();
	...
	private Vector<IterativeLimitedDepthFirstSearchElement> ruledNeighbors(Vector<String> rules,
			IterativeLimitedDepthFirstSearchElement ildfsElement) {
		...
		for (int i = rules.size() - 1; i >= 0; i--) {
			...
			for (String neighborCode : ildfsElement.getNeighbors().keySet()) {
				for (Edge edge : graphG.getE()) {
					if (ildfsElement.equalsStr(edge.getFrom()) && neighborCode.equals(edge.getTo())
							&& strRule.equals(edge.getOrderCode())) {
						if (!contains(ildfsClosed, neighborCode)) {
							elementToAdd = new IterativeLimitedDepthFirstSearchElement(vElements.get(neighborCode),
									ildfsElement.getLevel() + 1,
									ildfsElement.getNeighbors().get(neighborCode) + ildfsElement.getWeight());
