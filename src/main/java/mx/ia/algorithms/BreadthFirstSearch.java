...
public class BreadthFirstSearch {
	...
	public Graph resolve(String from, String to) {
		...
		// cerrados y no se encuentre exactamente igual en abiertos ()
		...
		while (!bfsOpened.isEmpty()) {
			...
			if (rules != null) {
				...
				/*
				 * Otherwise create and add all the not closed neighbor nodes which route hasn't
				 * been added to the opened vector
				 */

				for (String neighborCode : element.getNeighbors().keySet()) {
					if (!contains(bfsClosed, neighborCode) && !contains(bfsOpened, neighborCode)) {
						/*
						 * If the neighbor isn't closed (marked), and route is not open we can continue
						 */
						elementToAdd = new BreadthFirstSearchElement(vElements.get(neighborCode),
								element.getLevel() + 1, element.getNeighbors().get(neighborCode) + element.getWeight());
...
//		System.out.save();
	...
	private Vector<BreadthFirstSearchElement> ruledNeighbors(Vector<String> rules,
			BreadthFirstSearchElement bfsElement) {
				...
				for (Edge edge : graphG.getE()) {
					if (bfsElement.equalsStr(edge.getFrom()) && neighborCode.equals(edge.getTo())
						...
						if (!contains(bfsClosed, neighborCode)
								&& !containsRoute(bfsOpened, neighborCode, bfsElement.getVertex().getCode())) {
							elementToAdd = new BreadthFirstSearchElement(vElements.get(neighborCode),
									bfsElement.getLevel() + 1,
									bfsElement.getNeighbors().get(neighborCode) + bfsElement.getWeight());
	...
	private boolean containsRoute(Vector<BreadthFirstSearchElement> v, String strActual, String strPrev) {
		for (BreadthFirstSearchElement breadthFirstSearchElement : v)
			if (breadthFirstSearchElement.equalsStr(strActual)
					&& breadthFirstSearchElement.getPrevious().equalsStr(strPrev))
				return true;
		return false;
	}
