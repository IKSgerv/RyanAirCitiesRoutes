	...
	private Graph quitLonely(Graph graphG) {
		...
		for (int i = 0; i < graphG.getV().size(); i++) {
			...
			for (int j = 0; j < graphG.getE().size(); j++) {
				if (graphG.getE().get(j).getFrom().equals(graphG.getV().get(i).getCode())
						|| graphG.getE().get(j).getTo().equals(graphG.getV().get(i).getCode())) {
					found = true;
				}
			}
			if (!found) {
...
//		System.out.save();
	...
	Edge getMin() {
		...
		for (int i = 0; i < graphG.getE().size(); i++) {
			...
			if ((graphT.getV().contains(from) && !graphT.getV().contains(to))
					|| (graphT.getV().contains(to) && !graphT.getV().contains(from))) {
