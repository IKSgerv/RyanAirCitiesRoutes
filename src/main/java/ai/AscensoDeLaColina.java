...
//			throw new IllegalArgumentException("Can not find the vertex: " + newTo);
		...
		while(!dfsOpened.isEmpty()){
			...
			dfsOpened.addAll(heuristicNeighbors(element, vElements.get(to), angle));
...
//		System.out.save();
	...
	private Vector<AscensoDeLaColinaElement> heuristicNeighbors (AscensoDeLaColinaElement bfsElement, Vertex vTo, double angle){
...
class AscensoDeLaColinaElement implements Comparable<AscensoDeLaColinaElement>{
	...
	public double heuristicValuer(Vertex vTo){
		...
		System.out.println("::: angleP: " + angle2 + "|angleT: " + angle);
		angle = Math.abs(angle2 - angle);
		System.out.println(":::" + vertex.getCode() + ":" + angle + "|" + distance);
		return (1 + angle) + (distance * distance);
	}
